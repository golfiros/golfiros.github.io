...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \draw[very thick,->]
            ...
            \end{luacode}
        \draw[ultra thick]
            plot[domain=1:2.5,samples=10]
                (\x,{1.2*ln(\x)})
            plot[domain=2.5:5.5,samples=20]
                (\x,{1.2*ln(\x)-0.5*pow(sin((\x-2.5)*180/(5.5-2.5)),2)})
            plot[domain=5.5:7,samples=5]
                (\x,{1.2*ln(\x)});
    \end{tikzpicture}
\end{figure}
Qualquer estado preparado com $x$ na região instável gera uma coexistência de
